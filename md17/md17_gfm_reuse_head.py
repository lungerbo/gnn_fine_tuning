#!/usr/bin/env python3
"""
MD17 reuse-head fine-tuning.
- Loads GFM checkpoint (backbone + head)
- Freezes backbone; keeps graph_shared + heads_NN trainable (reuse head)
- Global label normalization (reads label_stats.json)
- Sanity checks: y shape and forward check
"""
...
def freeze_backbone_keep_head(model):
    for name, p in model.named_parameters():
        keep = ("graph_shared" in name) or ("heads_NN" in name)
...
if __name__ == "__main__":
    args = parse_args()
    tag = f"md17_gfm_reusehead_{args.fraction}_{os.path.basename(args.split_dir)}_seed{args.seed}"
    setup_log(tag)
    ...
    try:
        with torch.no_grad():
            ...
            log(f"[SANITY] forward out shape={tuple(out.shape)} y shape={tuple(b.y.shape)}")
